...
#have a random number generator to pick a number between 1-1380 to select the chargers that will get power,
#keep picking a random numbers and selecting chargers until the number of chargepoints calculated has
#been reached for that timestep
    ...
    for day in week_day_numbers:
            for i in range(48):
                ...
                if TOTAL_power > GRID_LIMIT:
                    ...
                    #the vehicles in the vehicles_charging list are unaffected, their dataframes do not need to change at all
                    #BUT need to make power and energy 0 and recalcualte battery capacity for the vehicles that wont now be charging
                    #start by making a list of vehicles that need to do this for
...
old_charging_dataframe.to_csv(r'C:\Users\mckin\Desktop\Demand_Side_Management\DSM Scenario 1 Results\Vehicles Charging Log\DSM Scenario 1 - 12000 Limit - original_chargers.csv')
after_charging_dataframe.to_csv(r'C:\Users\mckin\Desktop\Demand_Side_Management\DSM Scenario 1 Results\Vehicles Charging Log\DSM Scenario 1 - 12000 Limit - after_chargers.csv')
...
with tqdm(total=1380)  as pbar:
    for car in car_input_dict:
        CarData = car_input_dict[car]
        newpath = r'C:\Users\mckin\Desktop\Demand_Side_Management\DSM Scenario 1 Results\12000 Limit'
        if not os.path.exists(newpath):
            os.makedirs(newpath)
        CarData.to_csv(r'C:\Users\mckin\Desktop\Demand_Side_Management\DSM Scenario 1 Results\12000 Limit\ '+str(car)+ '.csv')
        pbar.update(1)
